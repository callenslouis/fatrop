...
class ImplicitIpSearchDirTestReso : public ::testing::Test
{
protected:
    ImplicitIpSearchDirTestReso()
        : ocp(std::make_shared<ImplicitOcpTestProblem>()), nlp(std::make_shared<ImplicitNlpOcp>(ocp)),
          nlp_resto(std::make_shared<IpNlpResto<ImplicitOcpType>>(nlp)), info(nlp->problem_dims()),
          data(std::make_shared<IpData<ImplicitOcpType>>(nlp_resto)),
          ...
          resto_solver(std::make_shared<PdSolverResto<ImplicitOcpType>>(info, solver)),
          search_dir(data, resto_solver)
    {
        data->current_iterate().set_mu(1.0);
        data->current_iterate().set_dual_bounds_l(
    ...
    std::shared_ptr<IpNlpResto<ImplicitOcpType>> nlp_resto;
    ProblemInfo info;
    ...
    std::shared_ptr<PdSolverResto<ImplicitOcpType>> resto_solver;
    IpSearchDirImpl<PdSolverResto<ImplicitOcpType>, ImplicitOcpType> search_dir;
};
...
TEST_F(ImplicitIpSearchDirTestReso, SolveImplicitLinearSystem) { EXPECT_NO_THROW(search_dir.compute_search_dir()); }

TEST_F(ImplicitIpSearchDirTestReso, UpdateImplicitIterateAndCheckInfeasibility)
{
    LinsolReturnFlag ret = search_dir.compute_search_dir();
    EXPECT_EQ(ret, LinsolReturnFlag::SUCCESS);
