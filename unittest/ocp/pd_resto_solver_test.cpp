...
protected:
    ...
    VecRealAllocated D_x =
        VecRealAllocated(info.number_of_primal_variables + info.number_of_slack_variables_resto);
    VecRealAllocated D_eq = VecRealAllocated(info.number_of_eq_constraints);
    std::shared_ptr<AugSystemSolver<ImplicitOcpType>> solver =
        std::make_shared<AugSystemSolver<ImplicitOcpType>>(info);
    std::shared_ptr<PdSolverOrig<ImplicitOcpType>> pd_solver_sp =
        std::make_shared<PdSolverOrig<ImplicitOcpType>>(info, solver);
    PdSolverOrig<ImplicitOcpType> &pd_solver = *pd_solver_sp;
    PdSolverResto<ImplicitOcpType> pd_solver_resto = PdSolverResto<ImplicitOcpType>(info, pd_solver_sp);
