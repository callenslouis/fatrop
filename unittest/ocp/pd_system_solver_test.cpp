...
class ImplicitPdTest : public ::testing::Test
{
protected:
    // Create OcpDims object
    int K = 10;                                                   // Number of stages
    ...
    MatRealAllocated full_kkt_matrix =
        MatRealAllocated(info.number_of_primal_variables + info.number_of_eq_constraints,
                         info.number_of_primal_variables + info.number_of_eq_constraints);
    // LinearSystem<PdSystemType<OcpType>> lss{info};
    std::shared_ptr<AugSystemSolver<ImplicitOcpType>> solver = std::make_shared<AugSystemSolver<ImplicitOcpType>>(info);
    PdSolverOrig<ImplicitOcpType> pd_solver = PdSolverOrig<ImplicitOcpType>(info, solver);
    void SetUp()
    {
        x = 0;
        full_matrix_jacobian = 0.;
...
{
    LinearSystem<PdSystemType<ImplicitOcpType>> ls(info, jacobian, hessian, D_x, false, D_eq, sl, su,
                                           zl, zu, rhs_x, rhs_s, rhs_g, rhs_cl, rhs_cu);
    VecRealAllocated x_full(ls.m());
    VecRealAllocated rhs_save(ls.m());
    VecRealAllocated tmp(ls.m());
...
{
    LinearSystem<PdSystemType<ImplicitOcpType>> ls(info, jacobian, hessian, D_x, true, D_eq, sl, su,
                                           zl, zu, rhs_x, rhs_s, rhs_g, rhs_cl, rhs_cu);
    VecRealAllocated x_full(ls.m());
    VecRealAllocated rhs_save(ls.m());
    VecRealAllocated tmp(ls.m());
