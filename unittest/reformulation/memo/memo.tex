...
\end{equation}
where $\begin{bmatrix} \tilde{M}_2^1 & \tilde{M}_2^2 \end{bmatrix} = M_2 P_r^1$ with $\tilde{M}_2^1 \in \mathbb{R}^{n_g \times \rho}$ and $\tilde{M}_2^2 \in \mathbb{R}^{n_g \times n_x - \rho}$.

Continuing gaussian elimination, we can obtain
\begin{align}
    \hat{L} &\coloneqq \begin{bmatrix} K_1 & & \\ K_2 & \bm{I}_{n_x-\rho} & \\ K_3 & & \bm{I}_{n_g} \end{bmatrix}
    & &\text{such that} &
    \hat{L}^{-1} &= \begin{bmatrix} K_1^{-1} & & \\ -K_2 K_1^{-1} & \bm{I}_{n_x-\rho} & \\ -K_3 K_1^{-1} & & \bm{I}_{n_g} \end{bmatrix}
\end{align}
using $K_3 \coloneqq \tilde{M}_2^1 V_1^{-1}$, which leads to
\begin{equation}
    \hat{L}^{-1} 
    \begin{bmatrix} P_l^1 & \\ & \bm{I}_{n_g} \end{bmatrix} 
    H_u
...
\end{equation}
where $K_4 \coloneqq \tilde{M}_2^2 - K_3 V_2$. Because of the zero-row, we must perform row pivoting in order to continue the decomposition. Hence, define
\begin{equation}
...
\begin{equation}
    P_l^2 \begin{bmatrix} K_4 & M_3\end{bmatrix} P_r^2 = L_2 \begin{bmatrix} V_3 & V_4 \end{bmatrix}
\end{equation}
where $V_3 \in \mathbb{R}^{n_g \times n_x - \rho}$ and $V_4 \in \mathbb{R}^{n_g \times n_u}$. Note that if $\rho = n_x$, this decomposition reduces to a decomposition of $M_3$. Note that the permutation matrices $P_l^2$ and $P_r^2$ affect also $K_3$ and $V_2$, hence we define
\begin{align}
    \tilde{K}_3 &\coloneqq P_l^2 K_3\\
    \tilde{V}_2 &\coloneqq V_2 \begin{bmatrix} \left(P_r^2\right)_{11} & \left(P_r^2\right)_{12} \end{bmatrix}
\end{align}

Finally, the decomposition of $M$ is given by 
\begin{equation}
    \hat{P}_l H_u \hat{P}_r = 
...
\begin{algorithm}
    ...
    \State $\tilde{K}_3 \gets P_l^2 K_3$
    \State $\tilde{V}_2 \gets V_2 \begin{bmatrix} \left(P_r^2\right)_{11} & \left(P_r^2\right)_{12} \end{bmatrix}$
    \State $L \gets \begin{bmatrix} K_1 & & \\ \tilde{K}_3 & L_2 & \\ K_2 &  & \bm{I}_{n_x-\rho} \end{bmatrix}$
    \State $U \gets \begin{bNiceMatrix}[margin=8pt, cell-space-limits=2pt] V_1 & \Block[draw=black]{1-2}{\tilde{V}_2}\\ & V_3 & V_4 \\ & & \bm{0}_{n_x-\rho \times n_u} \end{bNiceMatrix}$
...
\begin{align}
    X_1 \coloneqq \begin{bmatrix} P_l^1 & \\ & \bm{I}_{n_g} \end{bmatrix}
    H_u
    \begin{bmatrix} P_r^1 & \\ & \bm{I}_{n_u}\end{bmatrix}
    &= \begin{bmatrix} P_l^1 M_1 P_r^1 & \\ M_2 P_r^1 & M_3 \end{bmatrix}\\
    &= \begin{bmatrix} L_1 U_1 & \\ M_2 P_r^1 & M_3 \end{bmatrix}\\
    &= \begin{bmatrix} K_1 V_1 & K_1 V_2 & \\ K_2 V_1 & K_2 V_2 &  \\ \tilde{M}_2^1 & \tilde{M}_2^2 & M_3 \end{bmatrix}\\
    X_2 \coloneqq \begin{bmatrix} \bm{I}_\rho & & \\ & & \bm{I}_{n_g} \\ & \bm{I}_{n_x-\rho} & \end{bmatrix}
    X_1
    &= \begin{bmatrix} K_1 V_1 & K_1 V_2 & \\ \tilde{M}_2^1 & \tilde{M}_2^2 & M_3 \\ K_2 V_1 & K_2 V_2 & \end{bmatrix}
\end{align}
Such that we can compute
\begin{align}
    \hat{P}_l H_u \hat{P}_r &= \begin{bmatrix} \bm{I}_\rho & & \\ & P_l^2 & \\ & & \bm{I}_{n_x-\rho} \end{bmatrix}
    X_2
    \begin{bmatrix}\bm{I}_\rho & & \\ & \left(P_r^2\right)_{11} & \left(P_r^2\right)_{12}\\ & \left(P_r^2\right)_{21} & \left(P_r^2\right)_{22} \end{bmatrix}\\
    &= \begin{bmatrix} K_1 V_1 & K_1 V_2 & \\ P_l^2 \tilde{M}_2^1 & P_l^2  \tilde{M}_2^2 & P_l^2  M_3 \\ K_2 V_1 & K_2 V_2 & \end{bmatrix}
    \begin{bmatrix}\bm{I}_\rho & & \\ & \left(P_r^2\right)_{11} & \left(P_r^2\right)_{12}\\ & \left(P_r^2\right)_{21} & \left(P_r^2\right)_{22} \end{bmatrix}\\
    &= \begin{bNiceMatrix}[margin=8pt, cell-space-limits=2pt] 
        K_1 V_1 & K_1 \tilde{V}_2 \\ 
        P_l^2 \tilde{M}_2^1 & P_l^2 \begin{bmatrix} \tilde{M}_2^2 & M_3 \end{bmatrix} P_r^2 \\ 
        K_2 V_1 & K_2 \tilde{V}_2 
    \end{bNiceMatrix}
