...
int main(){
    ...
    for (int i = 0; i < nb_runs; ++i) {
        ...
        // std::cout << "Pl_A:\n" << Pl_blocked1[i] << std::endl;
        // std::cout << "Pr_C:\n" << Pr_blocked2[i] << std::endl;
        gecp(nx[i], ng[i], A_blocked[i], 0, nx[i], B, 0, 0);
        // std::cout << "B^T:\n" << B.block(nx[i], ng[i], 0, 0) << std::endl;
        // std::cout << "Pr_A:\n" << Pr_blocked1[i] << std::endl;
        Pr_blocked1[i].apply_inverse_on_rows(r1, &B.mat(), 0);
        // std::cout << "rows of B (inversely) permuted with Pr_A (r = " << r1 << "):\n" << B.block(nx[i], ng[i], 0, 0) << std::endl;
        // std::cout << "Pl_C:\n" << Pl_blocked2[i] << std::endl;
        // Pl_blocked2[i].apply_inverse_on_cols(r2, &B.mat());
        Pl_blocked2[i].apply_on_cols(r2, &B.mat());
        // std::cout << "cols of B (inversely) permuted with Pl_C (r = " << r2 << "):\n" << B.block(nx[i], ng[i], 0, 0) << std::endl;

        // blasfeo_dtrsm_lutn(nx[i], ng[i], 1.0, &A_blocked[i].mat(), 0, 0, &B.mat(), 0, 0, &B_tilde.mat(), 0, 0);
