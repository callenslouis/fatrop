...
floor_height = functions['fkpos_ee'](q0)[2] - 0.1

tau_eq = get_full_equilibrium(functions, q0)
path_func = get_path_function(functions, q0, floor_height + 0.1)
...
for k in range(N):
    ...
    if with_accel_variables:
        ...
        qdd = functions['fd_contact'](q[:,rhs_idx], qd[:,rhs_idx], tau[:,k], floor_height, k, alpha, d)
    ...
    # true dynamics
    if with_accel_variables:
        opti.subject_to(tau[:,k] == functions['id_contact'](q[:,rhs_idx], qd[:,rhs_idx], qdd[:,k], floor_height, k, alpha, d))

    # inequalities
    opti.subject_to(-1 <= (sumsqr(s[:,k]) <= path_tol**2))
    for i in range(n):
        opti.subject_to(-tau_bound[i] <= (tau[i,k] <= tau_bound[i]))
    opti.subject_to(force_min <= (functions['normal_force'](q[:,k], qd[:,k], floor_height, k, alpha, d) <= force_max))

    if with_progress_variable:
        opti.subject_to(0.2*dt <= (dp[:,k] <= 2.0*dt))

    obj += 0*sumsqr(s[:,k]) + control_penalty*sumsqr(tau[:,k]) + 1e-4*sumsqr(dp[:,k] - dt) + sumsqr(tau[:,k] - tau_eq)
    if with_accel_variables:
        obj += sumsqr(qdd[:,k])
